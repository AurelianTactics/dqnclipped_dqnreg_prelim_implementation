...
from stable_baselines3.common.utils import get_linear_fn, polyak_update, dqn_clipped_loss, dqn_reg_loss
...
class DQN(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            if self.loss_type == self.DQN_CLIPPED_LOSS_CONSTANT:
                loss = dqn_clipped_loss(current_q, target_q, target_q_a_max, self.gamma)
            elif self.loss_type == self.DQN_REG_LOSS_CONSTANT:
                loss = dqn_reg_loss(current_q, target_q, self.dqn_reg_loss_weight)
            else:
                # standard DQN
